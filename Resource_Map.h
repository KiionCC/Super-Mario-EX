...
public:
	...
	Array *pLadderArray;
	// 当前主角在那个悬浮梯上
	SuspensionLadder *marioInLadder;

	// 记录当前屏幕中的怪物数组
	//Array *pEnemyInScreen;
	bool isMarioInLadder(Point marioPos, float &ladderLevel);

	Sprite *pMushSprite;  //当前被顶出的蘑菇精灵
	Point mushTileCoord;  // 当前蘑菇所在的位置

	Item *pItem; // 当前被顶的闪烁的问号
	...
	// 启动地图上所有的物件
	void launchLadderInMap();

	void update(float dt);
	// 停止怪物和子弹的更新，然后再停止自己的更新
	void stopUpdateForMarioDie();
